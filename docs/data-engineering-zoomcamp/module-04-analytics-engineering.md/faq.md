...
Example: select * from {% raw %}{{ source('staging',<your table name in the database>') }}{% endraw %}
...
Using safe\_cast returns NULL instead of throwing an error. So use safe\_cast from dbt\_utils function in the jinja code for casting into integer as follows:
 ...
 {% raw %}
{{ dbt_utils.safe_cast('ehail_fee', api.Column.translate_type("integer")) }}
{% endraw %}
...
{% raw %}
{{ dbt.safe\_cast("PULocationID", api.Column.translate\_type("integer")) }} {% endraw %} as pickup\_locationid
